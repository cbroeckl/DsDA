...
DsDA<-function(
  ...
  setwd(choose.dir(caption = "Select Project (.PRO) directory"))
  ...
  cat("You may now start the WRENS script", '\n', '\n')
  cat("please ensure that interscan delay is set to 'manual' and 0.03 seconds", '\n')
  ...
  repeat {
    ...
    cat("waiting for idle .raw file", '\n')
    startwhile <-Sys.time()
    currentwhile<-Sys.time()
    while(length(idle) < 1) {
      Sys.sleep(10)
      raw<-list.files(path="./Data", pattern = '[rR][aA][wW]')
      raw<-raw[!grepl(fileIgnore, raw)]
      currenttime<-Sys.time()
      times<-sapply(1:length(raw), FUN=function(x) difftime(currenttime, 
                                                            file.info(paste("Data/", raw[x], "_FUNC001.DAT", sep='/'))$ctime, 
                                                            units="secs") )
      base<-gsub('.[rR][aA][wW]', "", raw)
    ...
    # base<-gsub('.[rR][aA][wW]', "", raw)
    cat("new .raw file written: converting to mzML", '\n')
    ...
    if(length(idle)>0) {
      ...
      if(length(do)>0) {
        ...
        c<-Sys.time()
        ## first remove MSe data if this is an MSe file.
        unlink("Data/tmp.raw", recursive=TRUE)
        Sys.sleep(2)
        dir.create("Data/tmp.raw")
        success<-file.copy(from=paste0("Data/", raw[do],"/", list.files(path=paste0("Data/", raw[do]))), to=paste0("Data/tmp.raw"))
        if(any(!success)) {stop("failed to copy data to tmp directory: ", base[do], '\n'); }
        Sys.sleep(2)
        extern<-readLines(paste0("Data/tmp.raw/", "_extern.inf"))
        f<-grep("Function Parameters", extern)
        if(length(f)>2) {
          if(grepl(extern[2],  "TOF SURVEY FUNCTION")) {
            files<-list.files(path=paste0("Data/", raw[do]))
            rmfile<-grep("_FUNC002.", files)
            file.remove(paste0("Data/tmp.raw/", files[rmfile]))
            msscantime<-2*msscantime
          }
        }
        ...
        if(lockmassrefine){
          system(
            # cat(
            paste(
              ' \"C:\\Program Files\\ProteoWizard\\ProteoWizard 3.0.10246\\msconvert.exe\" ',
              paste0(gsub("/", "\\\\", getwd()), "\\Data\\", "tmp.raw"),
              paste0("--", "filter"),
              paste0('\"', paste("lockmassRefiner", "mz=556.27712", "tol=0.3"), '\"'),
              "-o", paste0(gsub("/", "\\\\", getwd()),'\\mzML\\'),
              "--outfile", base[do],
              sep= " "
            )
          )
        ...
        xsraw<-xcmsRaw(paste0(gsub("/", "\\\\", getwd()),'\\mzML\\', base[do], ".mzML"), includeMSn=TRUE)
        f<-paste0(gsub("/", "\\\\", getwd()),'\\mzML\\', base[do], ".mzML")
        lg<-0
        ...
        rm(mtch)
        d<-Sys.time()
        cat('\n', "file", f, "processed in", round(as.numeric(difftime(d, c, units="mins")), digits=2), "minutes", '\n')
        a<-Sys.time()
...
out<-DsDA(
  ...
  tofMult=16.625,
  repeatMSMSfill=4,
  schedule.file='C:/Data/schedule.csv',
  minPeaks=100,
